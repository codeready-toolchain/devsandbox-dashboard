...
You can configure, if you need to use a specific backend, in the `app-config.local.yaml` file:
...
This is what you will see when the build fails: ![Known Issues](./images/known_build_issue.png)

This usually happens because podman doesn't have enough dedicated RAM to build the plugin image. The solution is to configure your local podman instance with enough resources.
